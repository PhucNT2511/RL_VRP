...
def train(actor, critic, task, num_nodes, train_data, valid_data, reward_fn, render_fn, 
    ...
    for epoch in range(10):  ##the number of epochs
        actor.train()
        critic.train()
...
def train_vrp(args):
    ...
    num_customers, max_load, max_demand, locations, demands = read_input_file('/kaggle/input/vrp-rl5/project2_2/input.txt')
...
def test_vrp(input_file, actor):
   ...
   # Use actor model to exec tour_indices
    ...
    tour = tour_indices.squeeze().tolist()
    if tour[0] != 0:
      # Thêm phần tử 0 vào đầu danh sách
      tour.insert(0, 0)
    # Plot the points and tours
    fig, ax = plt.subplots()

    # Convert tensors to numpy for plotting
    depot = x0.cpu().numpy()
    points = static_data.cpu().numpy()
    
    # Plot all points
    ax.scatter(points[0, 0, :], points[0, 1, :], c='black', marker='o', label='Customer')
    ax.scatter(depot[0, 0], depot[0, 1], c='black', marker='s', label='Depot')
    ...
    for i in range(len(tour)-1):
            start_index = tour[i]
            end_index = tour[i + 1]
            start_point = points[0, :, start_index] if start_index != 0 else depot[0, :, 0]
            end_point = points[0, :, end_index] if end_index != 0 else depot[0, :, 0]
            
            ax.plot([start_point[0], end_point[0]], [start_point[1], end_point[1]], color=colors[tour_color_index], linestyle='-', marker='o')
    ...
    name = 'output.png'
    output_file = os.path.join(save_dir, name)
...
if __name__ == '__main__':
    ...
    test_vrp('/kaggle/input/vrp-rl5/project2_2/input.txt', actor)
